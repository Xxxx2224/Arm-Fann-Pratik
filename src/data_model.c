...
FannError FannDataset_create(FannDataset **fannDataset, size_t total_columns,
  ...
  if (fannDataset == NULL || total_columns != input_columns + output_columns ||
      input_columns == 0 || output_columns == 0 || total_columns == 0) {
...
FannError FannDataset_parse_csv(FannDataset **fannxDataset,
                                const char *const file_path,
                                unsigned int skip_rows, const char ayrac) {
  if (*fannxDataset == NULL || file_path == NULL || file_path[0] == '\0' ||
      ayrac == '\0') {
    return FANN_ERR_ARG;
  }
  ...
  FannDataset_parse(fannxDataset, file_handle, end_ptr, skip_rows, ayrac);

  return FANN_SUCCESS;
}

FannError FannDataset_parse(FannDataset **fanndataset, char *const file_handlex,
                            char *end_ptr, unsigned int skip_rows,
                            const char ayrac) {
  char *file_handle = file_handlex;
  __uint64_t rows = 0;
  __uint8_t colums = 0;
  FannDataset *fannDataset = *(fanndataset);
  char *left_ptr = file_handle;
  while (file_handle < end_ptr) {

    if (*file_handle == ayrac) {
      *file_handle = '\0';
      fannDataset->rows[rows].output[colums] = left_ptr;
      // while (left_ptr<file_handle)
      // {
      // }
      colums++;
    }
